...
class LAPDAccessor(object):
    ...
    def just_principals(self):
        """
        return just principle level authorities
        """
        non_principal_codes = ["CTY", "COMB", "SRA"]
        df = self._obj
        added = False
        if "local-authority-type" not in df.columns:
            df = df.la.get_council_info(["local-authority-type"])
            added = True
        principals = ~df["local-authority-type"].isin(non_principal_codes)
        if added:
            df = df.drop(columns=["local-authority-type"])
        return df.loc[principals]
    ...
    def to_higher(self,
                  ...
                  aggfunc="mean",
                  weight_on: Optional[str] = None,
                  comparison_column: str = "overlapping-la",
