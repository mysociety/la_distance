...
def gdf_chart(gdf: geopandas.GeoDataFrame) -> alt.Chart:
    """
    start off an altair chart using a geodataframe
    """
    return alt.Chart(gdf_to_data(gdf))
...
@lru_cache
def lower_tier_base_layer() -> alt.Chart:
    """
    ...
    base = alt.Chart(gdf_to_data(background_layer)).mark_geoshape(
        stroke='black',
        fill='lightgrey',
        strokeWidth=0
    ).encode(
    ...
    groups = alt.Chart(gdf_to_data(groups_layer)).mark_geoshape(
        stroke='black',
        fill=None,
        strokeWidth=1
    ).encode()
...
@lru_cache
def lower_tier_label_layer() -> alt.Chart:
    """
    ...
    labels_left = alt.Chart(gdf_to_data(labels_layer_left)
                            ).mark_text(align="left", **text_props)
    labels_right = alt.Chart(gdf_to_data(labels_layer_right)).mark_text(
...
def lower_tier_sandwich(tier_layer: alt.Chart) -> alt.Chart:
    """
