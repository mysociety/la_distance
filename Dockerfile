...
RUN mkdir /setup/src/la_distance && touch /setup/src/la_distance/__init__.py \
    && touch /setup/src/data_common/__init__.py \
    && export PATH="$HOME/.poetry/bin:$PATH" \
    && cd /setup/ && poetry install \
    && echo "/workspaces/la_distance/src/" > /usr/local/lib/python3.10/site-packages/la_distance.pth \
    && echo "/workspaces/la_distance/src/data_common/src" > /usr/local/lib/python3.10/site-packages/data_common.pth
