...
@cache_and_wrap
def get_la_df(include_historical: bool = False, index=False) -> pd.DataFrame:
    """
...
def get_la_with_leagues() -> pd.DataFrame:
    ...
    unitaries = ['Welsh unitary authority', 'Scottish unitary authority',
                 ...
                 'London borough', 'City corporation']
...
class LAPDAccessor(object):
    ...
    def to_multiple_higher(self,
                           columns: Optional[List[str]] = None,
                           aggfunc="mean",
                           weight_on: Optional[str] = None,
                           higher_levels: Optional[List[str]] = [
                               "county-la", "combined-authority"],
                           upgrade_absent: bool = False) -> pd.DataFrame:
        """
        If given values by primary authority, we may want the
        overlapping authorities
        ...
        f = partial(self.to_current, columns=columns, aggfunc=aggfunc,
                    weight_on=weight_on, upgrade_absent=False)
